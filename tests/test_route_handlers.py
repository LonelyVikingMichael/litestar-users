...
class TestLogin:
    def test_login(self, client: "TestClient") -> None:
        response = client.post("/login", json={"email": "admin@example.com", "password": "iamsuperadmin"})
        assert response.status_code == 201

    @pytest.mark.xfail
    def test_login_invalid_credentials(self, client: "TestClient") -> None:
        with pytest.raises(NotAuthorizedException):
            response = client.post("/login", json={"email": "admin@example.com", "password": "ijustguessed"})
            assert response.status_code == 401

    @pytest.mark.xfail
    def test_login_unverified(self, client: "TestClient") -> None:
        with pytest.raises(PermissionDeniedException):
            response = client.post("/login", json={"email": "unverified@example.com", "password": "notveryverified"})
